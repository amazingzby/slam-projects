...
int main() {
    // 准备数据
    //Frame 结构体成员：Eigen::Matrix3d Rwc，Eigen::Quaterniond qwc，Eigen::Vector3d twc，
    //                unordered_map<int, Eigen::Vector3d> featurePerId; // 该帧观测到的特征以及特征id
    vector<Frame> cameras;
    vector<Eigen::Vector3d> points;
    //20个特征数目，3个相机位置，假设每帧都能观测到所有的特征
    GetSimDataInWordFrame(cameras, points);
    Eigen::Quaterniond qic(1, 0, 0, 0);//四元数
    Eigen::Vector3d tic(0, 0, 0);//位移

    // 构建 problem
    Problem problem(Problem::ProblemType::SLAM_PROBLEM);

    // 所有 Pose
    //vertexPose 为位姿顶点,继承pose类，重新定义了加法：位移相加，四元数相乘
...
//            vertexCam->SetFixed();
        ...
        problem.AddVertex(vertexCam);//problem将vertexCam添加进顶点
        vertexCams_vec.push_back(vertexCam);//将vertexCam加进vertexCams_vec
    }
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        //通过输出验证，第0帧Rwc为单位矩阵,twc = 0,0,0,上句话可以理解为以第0帧为参考系
        Eigen::Vector3d Pw = points[i];
        Eigen::Vector3d Pc = cameras[0].Rwc.transpose() * (Pw - cameras[0].twc);//其实还是等于 Pw
        noise = noise_pdf(generator);
        double inverse_depth = 1. / (Pc.z() + noise);//深度z加上噪声，再转换为逆深度
//        double inverse_depth = 1. / Pc.z();
        noise_invd.push_back(inverse_depth);

        // 初始化特征 vertex
        //逆深度顶点VertexInverseDepth，继承vertex，仅优化逆深度信息
        shared_ptr<VertexInverseDepth> verterxPoint(new VertexInverseDepth());
        VecX inv_d(1);
        inv_d << inverse_depth;
        verterxPoint->SetParameters(inv_d);
        problem.AddVertex(verterxPoint);//problem将vertexPoint添加进顶点
        allPoints.push_back(verterxPoint);//加进allPoints
