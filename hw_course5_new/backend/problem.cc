...
void Problem::SetOrdering() {
    ...
    //verticies_ :all vertices 为map类型，每个元素为一个pair,定义如下
    //typedef std::map<unsigned long, std::shared_ptr<Vertex>> HashVertex;
    int tempNum = 1;
    for (auto vertex: verticies_) {
        //通过打印输出，有3个6维vertex，20个1维vertex，共38
...
void Problem::MakeHessian() {
    TicToc t_h;
    // 直接构造大的 H 矩阵
    //size = 38,3 camera * 6 + 20 inverse depth
    ulong size = ordering_generic_;
    MatXX H(MatXX::Zero(size, size));
    VecX b(VecX::Zero(size));

    //edges_类型：typedef std::unordered_map<unsigned long, std::shared_ptr<Edge>> HashEdge;
    //edges_.size() = 40, 2 * 20,2为两个Frame分别相对于第0帧，20为20个特征点
    for (auto &edge: edges_) {

        //EdgeReprojection类中关于残差，Jacobians计算
...
//        delta_x_ = PCGSolver(H, b_, H.rows() * 2);
        ...
        int reserve_size = ordering_poses_;//保留位姿信息
        int marg_size = ordering_landmarks_;//margin掉路标点
