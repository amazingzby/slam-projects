...
void EdgeReprojection::ComputeResidual() {
    ...
    VecX param_i = verticies_[1]->Parameters();//Frame vertex，获取位移，旋转信息
    ...
    //pts_i_和pts_j_分别为0帧和第j帧对应的特征点
    //pts_i_ = [   0.29598,-0.0734253,1],inv_dep_i = 0.240649
    //pts_camera_i = [1.22993,-0.305114,4.15543]
    Vec3 pts_camera_i = pts_i_ / inv_dep_i;
    //在main函数中，qic，qic被定义为单位矩阵和0平移
    Vec3 pts_imu_i = qic * pts_camera_i + tic;
    Vec3 pts_w = Qi * pts_imu_i + Pi;//第i个相机坐标系
    Vec3 pts_imu_j = Qj.inverse() * (pts_w - Pj);//从i到j
    Vec3 pts_camera_j = qic.inverse() * (pts_imu_j - tic);//j从imu到camera
