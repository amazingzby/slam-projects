...
//处理图片数据，main方法PubImageData()调用此函数
//三种情况，经过简单处理后直接返回：未初始化，图像第一帧，时间戳异常（时间超过1s，本帧低于上一帧）
//正常情况下，读取图像数据，计算图像特征点，光流信息
    ...
    const Eigen::Vector3d &vAcc)
{
    ...
    m_buf.lock();
    imu_buf.push(imu_msg);//queue数据结构
    m_buf.unlock();
    con.notify_one();
}
    ...
    while (bStart_backend)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++) 
            {
                ...
                int feature_id = v / NUM_OF_CAM;//相机个数，单目系统下NUM_OF_CAM=1
