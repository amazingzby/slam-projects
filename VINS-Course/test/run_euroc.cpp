...
void PubImageData()
{
	...
	while (std::getline(fsImage, sImage_line) && !sImage_line.empty())
	{
		...
		// cv::waitKey(0);
		//Sleep time:100000 microsecond,0.1s
...
int main(int argc, char **argv)
{
	...
	//std::shared_ptr<System> pSystem;
	pSystem.reset(new System(sConfig_path));//reset为shared_ptr的方法
	
	std::thread thd_BackEnd(&System::ProcessBackEnd, pSystem);
		
	// sleep(5);
	std::thread thd_PubImuData(PubImuData);//将IMU数据加进buffer

	std::thread thd_PubImageData(PubImageData);//图像数据buffer，计算图像特征点，光流信息
